<?php
class Custom_Walker_Nav extends Walker_Nav_Menu {

  public function start_lvl( &$output, $depth = 0, $args = array() ) {
    $indent = str_repeat("\t", $depth);
    if ( $depth === 0 ) {
      $output .= "\n{$indent}<div class=\"dropdown-menu box-style\" aria-labelledby=\"navbarDropdown{$depth}\">\n";
    }
  }

  public function end_lvl( &$output, $depth = 0, $args = array() ) {
    $indent = str_repeat("\t", $depth);
    if ( $depth === 0 ) {
      $output .= "{$indent}</div>\n";
    }
  }

public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
  $classes       = empty( $item->classes ) ? [] : (array) $item->classes;
  $has_children  = in_array( 'menu-item-has-children', $classes );
  $is_dropdown   = $has_children && $depth === 0;
  $is_submenu    = $has_children && $depth === 1;

  // Collect custom CSS classes (from menu editor)
  $custom_classes = [];
  if ( !empty($item->classes) && is_array($item->classes) ) {
    foreach ( $item->classes as $class ) {
      // Skip WordPress system classes (menu-item-###, menu-item-type-post_type, etc.)
      if ( strpos( $class, 'menu-item' ) !== 0 && $class !== 'menu-item-has-children' ) {
        $custom_classes[] = sanitize_html_class( $class );
      }
    }
  }

  /* ---------- LEVEL 1 (top) ---------- */
  if ( $depth === 0 ) {
    $link_classes = array_merge(['nav-link'], $custom_classes);
    $link_class_attr = esc_attr( implode( ' ', array_unique( $link_classes ) ) );

    $output .= '<li class="nav-item' . ( $is_dropdown ? ' dropdown' : '' ) . '">';
    $output .= '<div class="dropdown-wrapper">';
    $output .= '<a class="' . $link_class_attr . '" href="' . esc_url( $item->url ) . '">' . esc_html( $item->title ) . '</a>';

    if ( $is_dropdown ) {
      $output .= $this->get_toggle_svg();
    }
    $output .= '</div>';

  /* ---------- LEVEL 2 (dropdown) ---------- */
  } elseif ( $depth === 1 ) {
    $link_classes = array_merge(['dropdown-item'], $custom_classes);
    if ( $is_submenu ) $link_classes[] = 'has-submenu';
    $link_class_attr = esc_attr( implode( ' ', array_unique( $link_classes ) ) );

    $output .= '<div class="dropdown-item-wrapper">';
    $output .= '<a class="' . $link_class_attr . '" href="' . esc_url( $item->url ) . '">' . esc_html( $item->title ) . '</a>';

    if ( $is_submenu ) {
      $output .= $this->get_toggle_svg();
      $output .= '<div class="dropdown-submenu">';
    }

  /* ---------- LEVEL 3+ (sub-items) ---------- */
  } elseif ( $depth >= 2 ) {
    $link_classes = array_merge(['dropdown-subitem'], $custom_classes);
    $link_class_attr = esc_attr( implode( ' ', array_unique( $link_classes ) ) );

    $output .= '<a class="' . $link_class_attr . '" href="' . esc_url( $item->url ) . '">' . esc_html( $item->title ) . '</a>';
  }
}


  public function end_el( &$output, $item, $depth = 0, $args = array() ) {
    $classes = empty( $item->classes ) ? [] : (array) $item->classes;

    // Close nested submenu
    if ( $depth === 1 && in_array( 'menu-item-has-children', $classes ) ) {
      $output .= '</div>'; // close .dropdown-submenu
    }

    // Close level 2 wrapper
    if ( $depth === 1 ) {
      $output .= '</div>'; // close .dropdown-item-wrapper
    }

    // Close top-level <li>
    if ( $depth === 0 ) {
      $output .= "</li>\n";
    }
  }

  private function get_toggle_svg() {
    return '<a class="dropdown-toggle" href="#" role="button" aria-haspopup="true" aria-expanded="false">'
         . '<svg xmlns="http://www.w3.org/2000/svg" width="21" height="21" viewBox="0 0 21 21" fill="none">'
         . '<path d="M4.33 9.48C3.52 8.68 4.09 7.3 5.23 7.3h10.11c1.14 0 1.71 1.38.9 2.18l-5.06 5.06a1.1 1.1 0 0 1-1.81 0L4.33 9.48Z" fill="#ffffff"></path>'
         . '</svg></a>';
  }
}
